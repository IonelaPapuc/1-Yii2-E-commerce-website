...
class CartItem extends \yii\db\ActiveRecord
{
    ...
    public static function getTotalPriceForUser($currUserId)
    {
        if (isGuest()) {
            ...
            $sum = CartItem::findBySql(
                "SELECT
                            SUM(ci.quantity * p.price) AS suma
                        FROM
                            cart_items ci
                        LEFT JOIN products p ON
                            p.id = ci.product_id  WHERE ci.created_by = :userId ", ['userId' => $currUserId])
                ->scalar();
        }
        return $sum;
    }
            ...
            $sum = CartItem::findBySql(
                "SELECT
                        ...
                        FROM
                            cart_items ci
                        LEFT JOIN products p ON
                            p.id = ci.product_id  WHERE ci.product_id = :id AND  ci.created_by = :userId ", ['id'=> $productId, 'userId' => $currUserId])
                ->scalar();
        }
        return $sum;
    }
...
public static function clearCartItems($currUserId)
{
    ...
    public function rules()
    {
        return [
            ...
            //  [['created_by'], 'exist', 'skipOnError'=> true, 'targetClass'=> User::className(), 'targetAttribute'=>['created_by']],
            //  [['product_id'], 'exist', 'skipOnError'=> true, 'targetClass'=> Product::className(), 'targetAttribute'=>['product_id']]
        ];
    }
